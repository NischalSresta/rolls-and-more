...
function App(){
 ...
 <div className="menu-layout"><aside className="menu-feature roll-feature reveal"><span className="tiny-label">MEET YOUR MATCH</span><h3>{category==='Burgers'?<>STACKED.<br/>SAUCY.<br/>SERIOUS.</>:category==='Wraps'?<>ROLL UP.<br/>BITE IN.<br/>REPEAT.</>:<>LOADED.<br/>CHEESY.<br/>ALL IN.</>}</h3><img src="/images/rolls.png" alt="Rolls and More’s toasted wrap promotion" loading="lazy"/><span className="feature-note">A LITTLE MORE OF WHAT YOU LOVE.</span></aside>
 <div className="menu-content"><div className="menu-tabs" aria-label="Menu categories">{Object.keys(menu).map(name=><button key={name} aria-pressed={category===name} onClick={()=>setCategory(name)}>{name}<sup>{menu[name].length.toString().padStart(2,'0')}</sup></button>)}</div><div className="menu-items" key={category} aria-live="polite">{menu[category].map(([name,price,description],i)=><article className="menu-item" key={name} style={{'--delay':`${i*40}ms`}}><div><h3>{name}{name==='Bipolar stack'&&<span className="item-tag">DOUBLE UP</span>}</h3><p>{description}</p></div><div className="menu-item-action"><span className="price">${price}</span><button className="add-item" aria-label={`Add ${name} to bag`} onClick={()=>addItem(name)}>+</button></div></article>)}</div><div className="combo"><div><strong>MAKE IT A COMBO.</strong><span>Fries + a 375 ml can. The whole deal.</span></div><b>+$5</b></div><p className="disclaimer">Prices from the supplied menu. Confirm current prices and dietary requirements in store.</p></div></div></section>

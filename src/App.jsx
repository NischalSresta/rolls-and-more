...
function App(){
 ...
 useEffect(()=>{
  ...
  const update=()=>{frame=0;if(reduced.matches)return;hero.current?.style.setProperty('--scroll',Math.min(window.scrollY,1000));if(story.current){const p=progress(story.current);story.current.style.setProperty('--p',p);story.current.style.setProperty('--mobile-shift',`${(p-.5)*18}px`);story.current.style.setProperty('--mobile-exploded-opacity',Math.max(0,1-p*1.55));story.current.style.setProperty('--mobile-stacked-opacity',Math.max(0,Math.min(1,(p-.28)/.6)));story.current.style.setProperty('--mobile-exploded-scale',1-p*.06);story.current.style.setProperty('--mobile-stacked-scale',.82+p*.18)}if(walk.current){const p=progress(walk.current);walk.current.style.setProperty('--walk',p);walk.current.style.setProperty('--entrance-scale',1+p*.18);walk.current.style.setProperty('--entrance-opacity',Math.max(0,1-p*1.35));walk.current.style.setProperty('--interior-scale',1.1-p*.08);walk.current.style.setProperty('--interior-opacity',Math.max(0,Math.min(1,(p-.12)*1.45)));walk.current.style.setProperty('--frame-opacity',Math.max(0,(p-.28)*1.6))}}
 ...
 <section className="walk-section" ref={walk} aria-label="A look inside the restaurant"><div className="walk-sticky"><div className="walk-visual"><img className="walk-entrance" src="/images/entrance.png" alt="The inviting Rolls and More entrance" loading="lazy"/><img className="walk-interior" src="/images/interior-immersive.png" alt="A warm, cinematic view inside the Rolls and More restaurant" loading="lazy"/></div><div className="walk-glow"/><div className="walk-frame" aria-hidden="true"><span/><span/><span/><span/></div><div className="walk-shade"/><div className="walk-copy"><span className="eyebrow">329 GUILDFORD ROAD</span><h2>COME ON IN.<br/><em>MAKE IT YOUR SPOT.</em></h2><p>A little Beirut. A lot of flavour.</p><span className="walk-status"><i/> OPEN THE DOOR. FEEL THE GLOW.</span></div><div className="walk-caption"><span>SCROLL TO STEP INSIDE <ArrowDown/></span><span>GUILDFORD · NSW</span></div></div></section>
 <section className="about section-pad" id="about"><div className="store-photo reveal"><img src="/images/interior-immersive.png" alt="The warm Rolls and More dining room and counter" loading="lazy"/><span className="photo-tag">YOUR NEW REGULAR SPOT <ArrowUpRight/></span></div><div className="about-copy reveal"><span className="eyebrow">RIGHT HERE ON GUILDFORD ROAD</span><h2>GOOD FOOD.<br/>YOUR PEOPLE.<br/><em>OUR PLACE.</em></h2><p>Lebanese street food, reimagined. Bold flavours, juicy meats, and loaded rolls packed with that real Beirut vibe. Find your next hunger fix at 329 Guildford Rd, Guildford NSW 2161.</p><a href={maps} target="_blank" rel="noreferrer" className="button red">FIND THE SPOT <ArrowUpRight/></a></div></section>
